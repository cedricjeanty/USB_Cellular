...
void test_harvest_empty_dir(void) {
    s_fs.add_dir("/sd");
    HarvestResult r = harvestFiles("/sd", "/sd/harvested", 1);
...
void test_harvest_single_file(void) {
    ...
    TEST_ASSERT_TRUE(s_fs.has_file("/sd/harvested/0001/data.csv"));
    TEST_ASSERT_EQUAL_STRING("hello,world\n", s_fs.get_content("/sd/harvested/0001/data.csv").c_str());
...
void test_harvest_skips_system_files(void) {
    ...
    TEST_ASSERT_EQUAL_UINT16(1, r.count);
    TEST_ASSERT_TRUE(s_fs.has_file("/sd/harvested/0001/data.csv"));
    TEST_ASSERT_FALSE(s_fs.has_file("/sd/harvested/0001/Thumbs.db"));
}
...
void test_harvest_subdirectory_flattening(void) {
    ...
    HarvestResult r = harvestFiles("/sd", "/sd/harvested", 1);
    TEST_ASSERT_EQUAL_UINT16(1, r.count);
    TEST_ASSERT_TRUE(s_fs.has_file("/sd/harvested/0001/logs__flight.bin"));
    // Source deleted
    TEST_ASSERT_FALSE(s_fs.has_file("/sd/logs/flight.bin"));
}
...
void test_harvest_multiple_files(void) {
    ...
    TEST_ASSERT_TRUE(s_fs.has_file("/sd/harvested/0001/file1.csv"));
    TEST_ASSERT_TRUE(s_fs.has_file("/sd/harvested/0001/file2.csv"));
    TEST_ASSERT_TRUE(s_fs.has_file("/sd/harvested/0001/file3.csv"));
    // Sources deleted
...
void test_harvest_sequential_folders(void) {
    ...
    TEST_ASSERT_TRUE(s_fs.has_file("/sd/harvested/0001/batch1.csv"));
    TEST_ASSERT_TRUE(s_fs.has_file("/sd/harvested/0002/batch2.csv"));
}
